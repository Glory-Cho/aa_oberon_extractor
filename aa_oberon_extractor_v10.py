...
def extract_json_in_overlay(frame) -> str | None:
    ...
    이 뷰는 팝오버가 아니라 전체 페이지 뷰이므로 문서 전체(body)를 스캔.
    본문 텍스트가 'JSON → Request → {...}' 순서라 마커 로직이 그대로 적용됨.
    """
    chunks: list[str] = []
...
def process_viz(frame, page, viz, panel_name: str, vi: int) -> int:
    ...
    if SKIP_EXISTING:
        prefix = f"{panel_name}__{viz_name}__"
        if any(f.name.startswith(prefix) for f in SAVE_DIR.iterdir() if f.is_file()):
    ...
    data = None
    for _ in range(EXTRACT_POLLS):
        data = extract_json_in_overlay(frame)   # 먼저 시도 → 준비됐으면 즉시 통과
        if data:
            break
        time.sleep(EXTRACT_POLL_S)
    # ── 추출 2: Copy to clipboard 버튼 경로 ──
    if not data:
    ...
    saved = 0
    if data:
        fname = SAVE_DIR / f"{panel_name}__{viz_name}__{ts_label}.json"
