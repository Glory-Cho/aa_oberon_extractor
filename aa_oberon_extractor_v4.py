...
def _click_first_visible(loc) -> bool:
    """locator 매칭 중 화면에 보이는 첫 요소 클릭(force). 성공 여부 반환."""
    try:
...
def extract_json_in_overlay(frame) -> str | None:
    """
    시간대 클릭 후 '같은 다이얼로그 안에' 열리는 Oberon XML 뷰에서
    JSON Request 추출. 화면에 안 보이는 스크롤 영역까지 포함해
    textarea(value) / pre / code / innerText 전부 수집 후 스캔.
    """
    try:
        chunks = frame.evaluate(
            """(sel) => {
                const out = [];
                for (const el of document.querySelectorAll(sel)) {
    ...
    if not chunks:
        return None
    # 'JSON Request' 마커가 있는 청크 우선, 그다음 긴 청크 순으로 시도
    ordered = sorted(
        chunks,
    ...
    for t in ordered:
        ...
        if objs:
            return objs[0]  # 마커 뒤 첫 오브젝트 = Request 본문
    return None
...
def process_viz(frame, page, viz, panel_name: str, vi: int) -> int:
    ...
    try:
        ...
        if tog.count() > 0:
            ...
            if label.startswith("Expand"):
                tog.first.click(timeout=8_000, force=True)
                viz_was_collapsed = True
                time.sleep(2.0)  # 데이터 로딩 대기
    except Exception:
        pass
    ...
    try:
        ...
        pass  # 불안정해도 force 클릭으로 진행

    dbg.first.click(timeout=10_000, force=True)
    time.sleep(1.2)

    overlay = frame.locator(SEL["overlay"])

    # ── 1단계: 'Freeform Table' 메뉴 ──
    ...
    try:
        ts.nth(idx).click(timeout=10_000, force=True)
    except Exception as e:
        ...
        page.keyboard.press("Escape")
        return 0
    time.sleep(1.8)  # Oberon XML 뷰 렌더 대기

    # ── 추출: 같은 다이얼로그 내부에서 (기본) / 새 창 폴백 ──
    data = extract_json_in_overlay(frame)
    if not data:
    ...
    if viz_was_collapsed:
        try:
            viz.locator(".subpanel-collapse-expand-button").first.click(
